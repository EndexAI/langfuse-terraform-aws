...
variable "app_version" {
  description = "Langfuse application version (Docker image tag) to deploy, e.g. \"4.30.0\". Defaults to the latest Langfuse release at the time this module version was published. The AI features require >= 4.25. That floor is written without a patch component on purpose, so update-langfuse-versions.yml cannot rewrite it when it moves this default. See https://github.com/langfuse/langfuse/releases."
  type        = string
  default     = "4.30.0"
}
